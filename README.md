...
As can be seen on the top, the galaxy is first rendered to a texture (most likely larger than the window resolution, due to super sampling configuration). Next, a galaxy background is generated by subsequently subsampling and blurring the original image and combining the different stages. This is quite similar to common bloom implemenations. The blurred galaxy background is then combined with the hi-res raw image of the galaxy in a dedicated shader. The final galaxy representation is eventually handed over to super sampling anti aliasing (SSAA), which low-pass filters the image and subsamples it to the final resolution.
